...
from hatch_cython.utils import autogenerated, memo, plat
...
def filter_ensure_wanted(wanted: Callable[[str], bool], tgts: list[str]):
    return list(
        filter(
            wanted,
            tgts,
...
class CythonBuildHook(BuildHookInterface):
    ...
    def exclude_spec(self) -> PathSpec:
        return PathSpec.from_lines(pathspec.patterns.GitWildMatchPattern, self.options_exclude)

    @property
    @memo
    def include_spec(self) -> PathSpec:
        return PathSpec.from_lines(pathspec.patterns.GitWildMatchPattern, self.options_include)

    @property
    @memo
    def exclude_compiled_src_spec(self) -> PathSpec:
        return PathSpec.from_lines(pathspec.patterns.GitWildMatchPattern, self.options_exclude_compiled_src)

    @property
    @memo
    def include_compiled_src_spec(self) -> PathSpec:
        return PathSpec.from_lines(pathspec.patterns.GitWildMatchPattern, self.options_include_compiled_src)
    ...
    def included_files(self):
        included_files = []
        for relative_path in {f.relative_path for f in self.build_config.builder.recurse_selected_project_files()}:
            if self.path_is_wanted(relative_path):
                if any(relative_path.endswith(ext) for ext in self.templated_extensions):
                    relative_path = os.path.splitext(relative_path)[0]  # noqa: PLW2901
    ...
    def excluded_compiled_src_files(self):
        ...
        for relative_path in {f.relative_path for f in self.build_config.builder.recurse_selected_project_files()}:
            if any(
                relative_path.endswith(ext) for ext in self.precompiled_extensions
            ) and self.path_is_wanted_excluded_compiled_src(relative_path):
    ...
    @property
    def grouped_included_files(self) -> list[ExtensionArg]:
        ...
        for norm in self.normalized_included_files:
            ...
            alias: Optional[str] = self.options.files.matches_alias(root)
    ...
    def get_aliased_path(self, path: str) -> str:
        ...
        alias: Optional[str] = self.options.files.matches_alias(path_alias_notation)
    ...
    def _glob_files(self, extensions: list[str], except_extra: bool, apply_aliases: bool = False) -> RelAbsPathMap:
        ...
        for included_file in self.included_files_without_extension:
            if apply_aliases:
                included_file = self.get_aliased_path(included_file)  # noqa: PLW2901
    ...
    @property
    @memo
    def included_files_without_extension(self) -> list[str]:
    ...
    def clean(self, versions: list[str]) -> None:
        files_to_remove = (
            list(self.autogenerated_files.values())
            + list(self.intermediate_files.values())
    ...
    def initialize(self, version: str, build_data: dict[str, Any]) -> None:
        ...
        if len(self.excluded) > 0:
            ...
            private_attr_name = "_BuilderConfig__exclude_patterns"
    ...
    def finalize(self, version: str, build_data: dict[str, Any], artifact_path: str) -> None:
        ...
        try:
            with (
                zipfile.ZipFile(artifact_path, "r") as zin,
                zipfile.ZipFile(tmp_path, "w", compression=zipfile.ZIP_DEFLATED) as zout,
            ):
                # Find RECORD
                record_name = next((n for n in zin.namelist() if n.endswith(".dist-info/RECORD")), None)
                if record_name is None:
                    msg = "Could not find *.dist-info/RECORD in wheel"
                    raise RuntimeError(msg)

                written = {}  # name -> (hash, size)

                for item in zin.infolist():
                    name = item.filename

                    # We'll regenerate RECORD at end
                    if name == record_name:
                        continue
                    ...
                    written[name] = (f"sha256={b64}", str(len(data)))

                # Regenerate RECORD
                out = io.StringIO()
                w = csv.writer(out, lineterminator="\n")
                for name in sorted(written.keys()):
                    w.writerow([name, written[name][0], written[name][1]])
                w.writerow([record_name, "", ""])
                zout.writestr(record_name, out.getvalue().encode("utf-8"))
